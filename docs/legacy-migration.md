...
**目前 gate：`BLOCKED_BY_IMPLEMENTATION_GAP`。** 沒有 migration runner，也沒有可用的 Plan 歸屬證據。
...
```bash
cd server
# production（Turso 憑證只放在執行環境，不進 repo）
...
只有 `SELECT`，不呼叫 `initSchema()`，`writes_performed` 恆為 0。輸出 JSON，含：
...
- `review_groups` — 給人看的清單。**刻意不叫 plan_candidates**：它不是分群建議
