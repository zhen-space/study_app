...
# Part 6：2C-P6-B — 科目先後順序

## 59. 產品語意（只有一句）

使用者排序「數學 → 化學 → 英文」，代表排程演算法在其他 hard constraint
都滿足的前提下，**優先讓前面的科目較早取得排程位置**；但**不要求**數學
全部完成才能開始化學。
...
「一定要讀完數學才能開始化學」是 dependency constraint，是另一件事，
之後獨立做。這一支不碰。

## 60. 傳輸形狀

`POST /api/schedule/preview` 的 `subject_order`：科目 id 由前到後的陣列。

```json
{ "subject_order": [12, 7, 3] }
```

- **結構化欄位，不是自然語言。** 之後 AI 也是把使用者的話映射成這個形狀
  送進來，不會再長出第二套語意。
- **名次是相對的**：沒被列到的科目排在列到的後面，彼此維持原本的順序。
- **不合法的內容當作沒指定**，不報錯。排序偏好是偏好，不該擋住整個排程。
- **沒指定就完全不送這個欄位**，後端行為與以前逐位元組相同。

## 61. 實作只動「順序」，不動日期

`buildQueue` 把各科的佇列照 `subject_order` 排好。這一個改動同時影響兩件事，
而且兩件都只是順序：

1. **輪流佇列裡誰先出手** —— 各科仍然每天輪流，只是輪流的起點照使用者排的
2. **`distribute` 的 buckets** 是照佇列第一次出現的順序建立的，主輪逐日發放時
...
那些會破壞既有的平均分配（§ pace/rate 那一整套），而且離 dependency 只剩一步。

### 預設順序不能被偷偷改掉

沒給 `subject_order` 時，各科的順序是 `Object.keys` 對整數型 key 的列舉順序
（科目 id 遞增）。排序必須是 stable sort，同名次維持原順序，否則「沒指定」
的情況也會被改掉——那就不是新增功能，是改掉既有行為。

## 62. UI

排程精靈第 2 步「排程條件」裡的「先讀哪一科？」，**只有多科時出現**。

- 用**上下移動**而不是拖曳：HTML5 drag 在手機上不可靠，而這一頁主要在手機上用
- 第一次移動時才把當下顯示的順序整個定下來；在那之前是「使用者沒指定」
- 「取消指定順序」回到沒指定
- 下方「各科的章節要打散還是照順序？」跟著同一個順序列出，否則會讓人
  以為剛排的順序沒生效
- 說明文案直接寫明這**不是**「前一科讀完才能開始下一科」

順序會存進 `scheduleConditions:plan:<id>` 快照（白名單多一個 `subjectOrder`），
重排時沿用同一個排法——不能每次重排都換一次順序。

## 63. 契約測試矩陣（2C-P6-B）

| # | 測項 | 守什麼 |
|---|---|---|
| 1 | 依 `subject_order` 決定每天誰先出手 | §61 |
| 2 | 優先的科目整體排到比較早的時段 | §59 |
| 3 | 反轉順序會產生不同的排程 | 不是擺設 |
| 4 | 沒指定時行為不變，**而且**預設仍是科目 id 遞增 | §61 |
| 5 | 只列一部分科目：沒列到的排在後面 | §60 |
| 6 | 不合法的 `subject_order` 不擋住排程 | §60 |
| 7 | 最後順位的科目仍在前半段就開始 | §59 priority ≠ dependency |
| 8 | 三科都排得完，沒有人因為順位靠後就排不進去 | §59 |
| 9 | 單科時 UI 不出現這個設定 | §62 |
| 10 | 沒排過 → request 完全沒有 `subject_order` | §60 |
| 11 | 排過 → 送出的順序就是畫面上的順序 | §62 |
| 12 | 「取消指定順序」後又回到不送 | §62 |
| 13 | 順序存進條件快照，重排沿用 | §62 |
