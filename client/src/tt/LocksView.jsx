...
export default function LocksView({ tasks=[] }) {
  ...
  return <div className="main"><PageHeader title="排程鎖定" subtitle="鎖定後，重新排程不會改動這些安排"/><div className="main-body">
    ...
    {type==='task'?<select value={task} onChange={e=>setTask(e.target.value)}><option value="">選擇已排入時間的任務</option>{tasks.filter(t=>t.plan_id&&!t.deleted&&!t.completed&&t.due_date).map(t=><option key={t.id} value={t.id}>{t.title}</option>)}</select>:<><input type="date" value={date} onChange={e=>setDate(e.target.value)}/>{type==='time'&&<div className="row"><input type="time" value={start} onChange={e=>setStart(e.target.value)}/><span>至</span><input type="time" value={end} onChange={e=>setEnd(e.target.value)}/></div>}</>}<Button variant="primary" block onClick={add}>鎖定</Button></SurfaceCard>
