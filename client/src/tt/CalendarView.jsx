...
export default function CalendarView({ tasks, reload, lists = [], addIntent = null, onAddIntentHandled }) {
  ...
  //
  // 差別不只是介面：星期課表的星期對應由伺服器的結構層依「絕對欄位位置」決定，
  // 模型只負責讀格子。低信心／有缺欄時一定要使用者確認過才准匯入。
  // 整個辨識→檢視→確認→寫入的流程收在共用的 <TimetableImporter>，
  // Wizard 也用同一個元件——只有一條 timetable pipeline。
  const [ttReq, setTtReq] = useState(null);      // { payload, imageUrl }
      ...
      <div className="main-body">
        {ttReq && (
          <TimetableImporter
            ...
            onImported={imported => { setTtReq(null); loadEvents(); if (imported) alert(`已加入 ${imported} 堂課（每週重複）`); }}
          />
        )}
        {aiList && (
