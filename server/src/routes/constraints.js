...
router.post('/plans/:id/constraints/parse', async (req, res) => {
  ...
  try {
    const response = await new Anthropic().messages.create({
      ...
      system: `把學生的排程自然語言轉成 JSON。只能輸出 JSON，不可臆測。支援欄位：subject_order（科目 id 陣列）、exclude_weekdays（0=週日..6）、exclude_dates（YYYY-MM-DD）、date_window（{start_date,end_date}）、deadline（YYYY-MM-DD）、preferred_time_ranges（[{start_time,end_time}]）、min_session_minutes／max_session_minutes（30 到 240 的整數）、max_per_day（1 到 20 的整數）、spread／sequential_within_subject／one_per_day（boolean）、availability_override（[{date,start_time,end_time}]，只限指定日期）。strict_dependency 與任何無法對應的需求一律照原 key 放在 JSON，但不得假稱生效。科目：${JSON.stringify(lists)}。`,
